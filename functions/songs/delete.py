...
def toggle_delete_songs(root, cursor):
    for widget in root.winfo_children():
        widget.pack_forget()
    
    label = tk.Label(root, text="Delete from songs form", font=('Arial', 16, 'bold'), bg='gray')
...
def submit_delete(cursor, delete_entry):
    ...
    delete_songs(cursor, deleted)
    
    
def delete_songs(cursor, deleted):
    try:
        cursor.execute("DELETE from songs WHERE ID = ?", (deleted, ))
