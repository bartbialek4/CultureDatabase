...
def toggle_delete_books(root, cursor):
    for widget in root.winfo_children():
        widget.pack_forget()
    
    label = tk.Label(root, text="Delete from Books form", font=('Arial', 16, 'bold'), bg='gray')
...
def submit_delete(cursor, delete_entry):
    ...
    delete_books(cursor, deleted)
    
    
def delete_books(cursor, deleted):
    try:
        cursor.execute("DELETE from books WHERE ID = ?", (deleted, ))
