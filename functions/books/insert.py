...
def toggle_insert_books(root, cursor):
    for widget in root.winfo_children():
        widget.pack_forget()
        
    label = tk.Label(root, text="Insert into Books form", font=('Arial', 16, 'bold'), bg='gray')
...
def submit_insert_form(cursor, name_entry, author_first_name_entry, author_last_name_entry, genre_entry, yr_published_entry):
    ...
    insert_database_books(cursor, name, author_first_name, author_last_name, genre, yr_published)
        
    
def insert_database_books(cursor, name, author_first_name, author_last_name, genre, yr_published):
    cursor.execute("""
        INSERT INTO books(NAME, AUTHOR_FIRST_NAME, AUTHOR_LAST_NAME, GENRE, YR_PUBLISHED) 
        VALUES (?, ?, ?, ?, ?)
